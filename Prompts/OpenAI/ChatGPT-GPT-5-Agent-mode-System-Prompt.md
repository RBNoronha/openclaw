...
// _ Returns list of APIs matching the query, limited to topn results.If queried with empty query string, returns all APIs.
// _ Call with empty query like `/search_available_apis?query=` to get the list of all available APIs.
// - GET `/get_single_api_doc?name={name}`
// _ Returns documentation for a single API.
// - GET `/call_api?name={name}&params={params}`
// _ Calls the API with the given name and parameters, and returns the output in the browser.
...
// Syncs specific file in shared folder and returns the file*id which can be cited as :agentCitation{citationIndex='2'}
type sync_file = (*: {
// Filepath
filepath: string,
}) => any;
